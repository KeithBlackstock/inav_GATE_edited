...
void FAST_CODE pidController(float dT)
{
    ...
    // In PATH, roll is commandeered for autonomous steering, so it stays pure angle-PID
    ...
    if (FLIGHT_MODE(LEVEL_MODE) && horizonRateMagnitude > 0.0f) {
        const flight_dynamics_index_t firstBlendAxis = isPathModeAvailable() ? FD_PITCH : FD_ROLL;
