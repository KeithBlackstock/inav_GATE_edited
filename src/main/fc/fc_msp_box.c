...
static const box_t boxes[CHECKBOX_ITEM_COUNT + 1] = {
    ...
    { .boxId = BOXPATH,             .boxName = "PATH",              .permanentId = 74 },
...
#ifdef USE_GPS
    if (feature(FEATURE_GPS) && sensors(SENSOR_ACC)) {
        ADD_ACTIVE_BOX(BOXPATH);
    }
#endif
...
void packBoxModeFlags(boxBitmask_t * mspBoxModeFlags)
{
    ...
    CHECK_ACTIVE_BOX(IS_ENABLED(IS_RC_MODE_ACTIVE(BOXPATH)),           BOXPATH);
