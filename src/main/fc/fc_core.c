...
static bool isAccRequired(void) {
    return isModeActivationConditionPresent(BOXNAVPOSHOLD) ||
        ...
        isModeActivationConditionPresent(BOXPATH) ||
...
static void processPilotAndFailSafeActions(float dT)
{
    if (failsafeShouldApplyControlInput()) {
        ...
        if (FLIGHT_MODE(HEADFREE_MODE) && !isPathModeAvailable()) {
...
void processRx(timeUs_t currentTimeUs)
{
    ...
    if (sensors(SENSOR_ACC) && (!FLIGHT_MODE(MANUAL_MODE) || autoEnableAngle || isPathModeAvailable())) {
        if (IS_RC_MODE_ACTIVE(BOXANGLE) || autoEnableAngle) {
            ENABLE_FLIGHT_MODE(ANGLE_MODE);
        } else if (IS_RC_MODE_ACTIVE(BOXHORIZON)) {
            ENABLE_FLIGHT_MODE(HORIZON_MODE);
        } else if (IS_RC_MODE_ACTIVE(BOXLEVEL) || isPathModeAvailable()) {
    ...
    if (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE) || FLIGHT_MODE(LEVEL_MODE)) {
        LED1_ON;
    } else {
        LED1_OFF;
    }
...
static void applyThrottleTiltCompensation(void)
{
    if (STATE(MULTIROTOR)) {
        ...
        else if (systemConfig()->throttle_tilt_compensation_strength && (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE) || FLIGHT_MODE(LEVEL_MODE))) {
