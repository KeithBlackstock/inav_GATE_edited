...
void processRx(timeUs_t currentTimeUs)
{
    ...
    DISABLE_FLIGHT_MODE(ATTITUDE_MODE);
    ...
    if (sensors(SENSOR_ACC) && (!FLIGHT_MODE(MANUAL_MODE) || autoEnableAngle)) {
        ...
        } else if (IS_RC_MODE_ACTIVE(BOXATTITUDE)) {
            ENABLE_FLIGHT_MODE(ATTITUDE_MODE);
        } else if (STATE(AIRPLANE) && IS_RC_MODE_ACTIVE(BOXANGLEHOLD)) {
            ENABLE_FLIGHT_MODE(ANGLEHOLD_MODE);
        }
    }

    if (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE) || FLIGHT_MODE(ATTITUDE_MODE)) {
        LED1_ON;
    } else {
        LED1_OFF;
    }
